...
int BPF_PROG(sys_exit, struct pt_regs *regs, long ret) {
	/* HS_FALCO self-filter: drop events generated by the scap (falco) process before
	 * they touch the ring buffer. Otherwise the consumer thread's own syscalls refill
	 * the same rings it drains — a feedback loop that dominates drops under load. */
