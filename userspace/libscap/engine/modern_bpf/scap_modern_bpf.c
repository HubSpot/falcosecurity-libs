...
int32_t scap_modern_bpf__init(scap_t* handle, scap_open_args* oargs) {
	...
	/* HS_FALCO self-filter: publish our own tgid so the BPF programs can drop events
	 * generated by this process before they enter the ring buffer. Eliminates the
	 * self-amplifying feedback loop where the consumer thread's syscalls refill the
	 * same rings it drains. Userspace libsinsp also filters by m_self_pid, but only
	 * after ring-buffer space has already been consumed.
	 *
	 * Must be set *after* socket calibration: calibration emits and then consumes
	 * a socket() event from this process, which would otherwise be dropped here. */
